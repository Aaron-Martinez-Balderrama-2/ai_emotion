    ...
    try {
        ...
        if (!file_exists($txt_path)) {
            ...
            if (file_exists($audio_path)) {
                ...
                $cmd_whisper = "whisper \"$audio_path\" --model base --language es --output_format txt --output_dir \"$temp_dir\"";
                exec($cmd_whisper);
            }
        }
        
        $transcription_text = file_exists($txt_path) ? file_get_contents($txt_path) : "(No se pudo generar transcripción)";
